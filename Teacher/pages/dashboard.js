...
function renderDashboard() {
    ...
    container.innerHTML = `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="card">
                <div class="card-content flex flex-col items-center text-center p-6">
                    <div class="w-32 h-32 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center text-white mb-4" style="width: 8rem; height: 8rem; background: linear-gradient(135deg, #3b82f6, #1d4ed8); border-radius: 9999px; display: flex; align-items: center; justify-center; color: white; margin-bottom: 1rem;">
            ...
            <div class="card lg:col-span-2">
                ...
                <div class="card-content">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ${renderInfoItem('mail', 'Email Address', state.userInfo.email)}
                        ${renderInfoItem('phone', 'Phone Number', '+91 9876543220')}
                        ${renderInfoItem('graduation-cap', 'Qualification', 'PhD in Computer Science')}
                        ${renderInfoItem('book-open', 'Specialization', 'AI & Machine Learning')}
                        ${renderInfoItem('briefcase', 'Experience', '12 years')}
                        ${renderInfoItem('calendar', 'Joining Date', 'August 2012')}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mt-6">
            ${renderStatCard('Total Students', state.stats.totalStudents, 'users')}
            ${renderStatCard('Active Courses', state.stats.activeCourses, 'book-open')}
            ${renderStatCard('Pending Reviews', state.stats.pendingReviews, 'clipboard-list')}
            ${renderStatCard('Classes Today', state.stats.upcomingClasses, 'calendar')}
            ${renderStatCard('Avg Attendance', state.stats.averageAttendance + '%', 'trending-up')}
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div class="card">
                ...
                <div class="card-content space-y-3">
                    ...
                    `).join('')}
                    <a href="schedule.html" class="btn btn-outline w-full mt-4">View Full Schedule</a>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-content">
                    <div class="grid grid-cols-2 gap-4">
                        <a href="qr-code.html" class="btn btn-outline h-20 flex-col gap-2" style="height: 5rem;">
                            <i data-lucide="qr-code" class="w-6 h-6"></i> Generate QR
                        </a>
                        <a href="assignments.html" class="btn btn-outline h-20 flex-col gap-2" style="height: 5rem;">
                            <i data-lucide="plus" class="w-6 h-6"></i> New Assignment
                        </a>
                        <a href="grades.html" class="btn btn-outline h-20 flex-col gap-2" style="height: 5rem;">
                            <i data-lucide="bar-chart-3" class="w-6 h-6"></i> View Grades
                        </a>
                        <a href="schedule.html" class="btn btn-outline h-20 flex-col gap-2" style="height: 5rem;">
